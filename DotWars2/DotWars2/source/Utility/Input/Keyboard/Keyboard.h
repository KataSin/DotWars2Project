...
public:
	...
	/// <returns>指定されたキーが入力された瞬間のフラグ</returns>
	bool KeyTriggerDown(UINT keycode);
	/// <summary>
	/// 指定されたキーが入力されているか調べる
	/// </summary>
	/// <param name="keycode">キーコード</param>
	/// <returns>指定されたキーが入力されているかのフラグ</returns>
	bool KeyStateDown(UINT keycode);
	/// <summary>
	/// 指定されたキーが離れた瞬間か調べる
	/// </summary>
	/// <param name="keycode">キーコード</param>
	/// <returns>指定されたキーが離れた瞬間のフラグ</returns>
	bool KeyTriggerUp(UINT keycode);
	/// <summary>
	/// 指定されたキーが離れているか調べる
	/// </summary>
	/// <param name="keycode">キーコード</param>
	/// <returns>キーが離れているかのフラグ</returns>
	bool KeyStateUp(UINT keycode);
	/// <summary>
	/// いずれかのキーが入力された瞬間か調べる
	/// </summary>
	/// <returns>どれがのキーが押された瞬間のフラグ</returns>
	bool AnyTriggerDown();
	/// <summary>
	/// いずれかのキーが入力されているか調べる
	/// </summary>
	/// <returns>キーが入力されているかのフラグ</returns>
	bool AnyStateDown();
	/// <summary>
	/// いずれかのキーが離れているか調べる
	/// </summary>
...
private:
	void KeyUpdate(const char& key, int num);
	char mKeyCode[256];

	std::array<int, 256> mOnKey;
	std::array<int, 256> mOffKey;
};
